...
参考[文档](#一部署基于-http-协议的-bcos-node-proxy)在两台机器上（`192.168.0.1`和`192.168.0.2`）分别部署 Bcos-node-proxy 服务，两个服务均监听`8170`端口。记用户安装 Nginx 的操作目录为`/data/home/app/`。
...
```text
    upstream proxy {
        server 192.168.0.1:8170 weight=1;
        server 192.168.0.2:8170 weight=1;
    }
```
